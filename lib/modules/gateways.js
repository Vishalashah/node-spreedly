 ...
 */
Gateways.listGateways = function listGateways(params, cb) {
	params = params || '';
...
/**
 * Lists all gateways that have been provisioned in your Spreedly environment based on the options
 *
 * @class Spreedly
 * @module Gateways
 ...
 * @returns {Promise} Bluebird promise that is resolved with an array of gateway elements
 */
Gateways.listGatewaysWithOptions = function listGatewaysWithOptions(params,cb) {
	return this._doRequest({
		url: 'gateways.xml' + params
	})
		.then(util.extractRoot())
...
/**
 * List transactions of a gateway
 *
 * @class Spreedly
 * @module Gateways
 * @param {String|Object} gatewayToken A gateway token, or a gateway element containing a token of the gateway to redact
 ...
 * @returns {Promise} Bluebird promise that is resolved with a transaction element
 */
Gateways.gatewayTransactions = function gatewayTransactions(gatewayToken, params, cb) {
	// Allow caller to pass in gateway objects
	gatewayToken = gatewayToken.token || gatewayToken;
	...
	return this._doRequest({
		url: 'gateways/' + gatewayToken + '/transactions.xml' + params
	})
		.then(util.extractRoot())
		.nodeify(cb);
};
